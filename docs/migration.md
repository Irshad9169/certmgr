...
| 9 | The full env file itself, for everything not listed above (SMTP/Slack/webhook settings, DB connection details you're keeping, cron schedules, retention settings, etc.) | `/etc/certmgr/certmgr.env` or `.env` | Faster to carry the whole file forward and adjust host-specific bits (see step 3) than to reconstruct it from `.env.example` defaults. |
...
```
Confirm the script logged `reusing provided CERTMGR_SECRETS_MASTER_KEY
(migration mode)` — if it instead says `secrets generated`, stop and fix this
before going any further; it means a new key was generated and the migration
would result in permanently undecryptable data once you copy the old
database/storage over.

### 3. Reconcile the rest of the env file
Diff the new host's generated env file against the one you saved in step 1.
Carry over anything you want preserved (SMTP/Slack/webhook config, custom
retention/cron settings) — but update anything genuinely host-specific:
`CERTMGR_DATABASE_URL`/`CERTMGR_REDIS_URL` if the DB/Redis host changed,
`CERTMGR_CORS_ORIGINS`/nginx `server_name` if the domain changed, and any
firewall/MariaDB grant that was scoped to the old server's IP.
